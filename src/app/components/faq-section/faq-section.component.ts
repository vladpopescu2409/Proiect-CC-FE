...
export class FaqSectionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.initializeFilteredFaqsByCategory();
    this.getFaqs();

  }

  initializeFilteredFaqsByCategory(): void {
    this.faqCategories.forEach(category => {
      this.filteredFaqsByCategory[category] = [];
    });
  }
  getFaqs() {
    this.faqsService.getFaqContent().subscribe((response) => { 
      console.log(response);
      // makes hhtp request - fetches the faqs - sends back the response
